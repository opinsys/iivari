...
Iivari is a digital signage system. The goal of the
...
Every display needs to have its own client computer that runs a
web browser implementation. The contents are stored in offline
storage that will be used to display slides when network connection is
unavailable.
...
The development of Iivari has been done in parallel with Puavo.
For ad-hoc standalone setups, this branch introduces an option
to disable LDAP and Puavo integration.
